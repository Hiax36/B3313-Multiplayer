...
def main():
    ### Memory Addresses ###
    livesAddress = 0x8033B21D #8 bit (signed for some reason) Verified
    MarioXAddress = 0x8033B1AC # 32 bit float Verified
    MarioYAddress = 0x8033B1B0 # 32 bit float Verified
    MarioZAddress = 0x8033B1B4 # 32 bit float Verified
    MarioHealthAddress = 0x8033B21E # 2 bytes ish Verified
    starCountAddress = 0x8033B21A # 16 bit Verified
    levelNumberAddress = 0x8032DDF8 # 16 bit Verified
    ...
    while True:
        ...
        for conn in connections:
            ...
            conn[0].send(MarioXAddress.to_bytes(4))
            playerPositions[conn[0]][0] = int.from_bytes(conn[0].recv(4),signed=False)

            #Y
            conn[0].send((4).to_bytes(1))
            conn[0].send(MarioYAddress.to_bytes(4))
            playerPositions[conn[0]][0] = int.from_bytes(conn[0].recv(4),signed=False)

            #Z
            conn[0].send((4).to_bytes(1))
            conn[0].send(MarioZAddress.to_bytes(4))
            playerPositions[conn[0]][0] = int.from_bytes(conn[0].recv(4),signed=False)
        ...
        for conn in connections:
            ...
            conn[0].send(levelNumberAddress.to_bytes(4))
            levelNums[conn[0]] = int.from_bytes(conn[0].recv(2),signed=False)
        ...
        for conn in connections:
            ...
            conn[0].send(areaNumberAddress.to_bytes(4))
            levelNums[conn[0]] = int.from_bytes(conn[0].recv(1),signed=False)
        ...
        if ENABLE_LIVELINK:
            for conn in connections:
                conn[0].send((2).to_bytes(1))
                conn[0].send(livesAddress.to_bytes(4))
                ...
                if tempLives != currentLives:
                    ...
                    for conn2 in connections:
                        if conn2 != conn:
                            ...
                            conn2[0].send(livesAddress.to_bytes(4))
        ...
        if ENABLE_HEALTHLINK or ENABLE_DEATHLINK:
            for conn in connections:
                conn[0].send((2).to_bytes(1))
                conn[0].send(MarioHealthAddress.to_bytes(4))
                ...
                if tempHealth != currentHealth:
                    ...
                    if ENABLE_HEALTHLINK:
                        ...
                        for conn2 in connections:
                            if conn2 != conn:
                                conn2[0].send((6).to_bytes(1))
                                conn2[0].send(MarioHealthAddress.to_bytes(4))
        ...
        for conn in connections:
            conn[0].send((3).to_bytes(1))
            conn[0].send(starCountAddress.to_bytes(4))
            tempStars = int.from_bytes(conn[0].recv(2),signed=False)
            if tempStars != currentStars:
                ...
                for conn2 in connections:
                    if conn2 != conn:
                        ...
                        conn2[0].send(starCountAddress.to_bytes(4))
                        conn2[0].send(currentStars.to_bytes(2))
        ...
        for conn in connections:
            ...
            recvAddr = None
            while True:
                recvAddr = int.from_bytes(conn[0].recv(4),signed=False)
                if recvAddr == 4294967295:
                    break
                newdata = int.from_bytes(conn[0].recv(4),signed=False)
                ...
                for conn2 in connections:
                    ...
                    conn2[0].send(recvAddr.to_bytes(4))
                    conn2[0].send(newdata.to_bytes(4))
        print("Done Syncing EEPROM")
        #time.sleep(1.0/ServerFrequency)
